...
def flask_client(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Any:
...
def test_home_page(client: Any) -> None:
    resp = client.get("/")
    assert resp.status_code == 200


def test_upload_updates_index(client: Any) -> None:
...
def test_chat_endpoint(client: Any) -> None:
    pdf_path = Path("pdfs") / "Sai Teja Resume.pdf"
    with pdf_path.open("rb") as f:
        client.post(
            "/upload",
